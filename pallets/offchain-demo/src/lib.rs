...
impl<T: Trait> Module<T> {
	...
	fn send_signed(block_number: T::BlockNumber) -> Result<(), Error<T>> {
		...
		for (acc, res) in &results {
			match res {
				Ok(()) => { debug::info!("off-chain send_signed: acc: {}| number: {}", acc, submission); },
				Err(e) => {
					debug::error!("[{:?}] Failed to submit signed tx: {:?}", acc, e);
					return Err(<Error<T>>::SendSignedError);
				}
			};
		}
...
impl<T: Trait> frame_support::unsigned::ValidateUnsigned for Module<T> {
	...
	fn validate_unsigned(call: &Self::Call) -> TransactionValidity {
		if let Call::submit_number_unsigned(block_num, number) = call {
			debug::info!("off-chain send_unsigned: block_num: {}| number: {}", block_num, number);
